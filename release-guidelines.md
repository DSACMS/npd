...
NPD uses [Calendar Versioning](https://calver.org/). Each release is associated with a [`git tag`](github.com/DSACMS/NPD/tags) of the form `YYYY.MM.DD`.

Given a version number in the `YEAR.MONTH.DAY` (eg., `YYYY.MM.DD`) format, the tag should correspond to a valid UTC date in the gregorian calendar. 
