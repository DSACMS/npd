## About the National Provider Directory

### Initial testing release: Building the foundation together

For years, provider directories have been fragmented, inconsistent, and burdensome for everyone who depends on them. Providers spend valuable time maintaining the same information across dozens of systems. Payers struggle to confirm details. Patients often cannot find accurate information about where to receive care.

The National Provider Directory (NPD) aims to fix that by creating a single, trusted source for provider and payer information across the U.S. healthcare system. The directory is being developed as a shared public resource, maintained collectively by CMS, providers, health plans, and other partners to ensure data stays accurate and up to date.
...
The next phases will add external data, connectivity details, and participation of CMS-Aligned Networks (health data networks that are voluntarily meeting the [CMS Interoperability Framework](https://www.cms.gov/health-technology-ecosystem/interoperability-framework) criteria).

### Long-term vision: A shared source of truth for healthcare

The long-term goal is to create a national, authoritative provider directory that reduces administrative burden, increases data accuracy, and improves patient access to care.

By consolidating the fragmented systems that exist today, the NPD will allow updates made by providers or health plans in one place to flow across the ecosystem. This shared responsibility model will reduce redundant reporting, eliminate confusion, and create cost savings for both the public and private sectors.
