...
--insert into npd.provider (individual_id, npi) select uuid_id as individual_id, id as npi from silver_nppes.npidetail where entity_type_code like '1';

--insert into npd.organization (id) select uuid_id from silver_nppes.npidetail where entity_type_code like '2';

--insert into npd.clinical_organization (organization_id, npi) select uuid_id as organization_id, id as npi from silver_nppes.npidetail where entity_type_code like '2';

--insert into npd.individual_to_name (individual_id, prefix, first_name, middle_name, last_name, suffix, name_use_id) select n.uuid_id as individual_id, prefix, first_name, middle_name, last_name, suffix, 1 as name_use_id from silver_nppes.npi_individual i left join silver_nppes.npidetail n on i.npi = n.npi;

insert into npd.organization_to_name (organization_id, name, is_primary) 
select organization_id, name, case when max(is_primary) = 1 then true else false end as is_primary 
from (select co.organization_id, provider_organization_name_legal_business_name as name, 1 as is_primary 
from bronze_nppes.main_file mf
inner join npd.clinical_organization co on co.npi = mf.npi::bigint
where provider_organization_name_legal_business_name not like ''
union
select co.organization_id, provider_other_organization_name as name, 0 as is_primary 
from bronze_nppes.main_file mf
inner join npd.clinical_organization co on co.npi = mf.npi::bigint
where provider_other_organization_name not like ''
union
select co.organization_id, provider_other_organization_name as name, 0 as is_primary 
from bronze_nppes.othername_file mf
inner join npd.clinical_organization co on co.npi = mf.npi::bigint
where provider_other_organization_name not like '') group by organization_id, name